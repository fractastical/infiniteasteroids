const InfiniteAsteroidsAnimation = (function () {
  ...
  function animate(currentTime) {
    ...
    let instruction;
    if (isMobile()) instruction = mobileInstructions[instructionPhase];
    else instruction = instructions[instructionPhase];
  ...
  class Particle {
    ...
    update() {
      ...
      if (distance > 0.5) {
        // Reduced distance threshold for stopping
  ...
  function getRandomEdgePosition() {
    ...
    switch (edge) {
      case 0:
        return { x: Math.random() * canvas.width, y: 0 }; // Top
      case 1:
        return { x: canvas.width, y: Math.random() * canvas.height }; // Right
      case 2:
        return { x: Math.random() * canvas.width, y: canvas.height }; // Bottom
      case 3:
        return { x: 0, y: Math.random() * canvas.height }; // Left
    }
  }
  ...
  function createTextPath() {
    ...
    const spacing = 12;
    const totalWidth =
      ctx.measureText(text).width + (characters.length - 1) * spacing;
    ...
    for (let y = 0; y < canvas.height; y += 1) {
      for (let x = 0; x < canvas.width; x += 1) {
        if (imageData.data[(y * canvas.width + x) * 4 + 3] > 128) {
          const charIndex = letterCenters.findIndex(
            (center) =>
    ...
    letterCenters.forEach((center, index) => {
      if (center.pixels.length === 0) {
        console.warn(
          `No pixels detected for letter at index ${index}. Skipping particle creation.`
        );
        return;
      }
      ...
      for (let i = 0; i < particlesPerLetter; i++) {
        const pixel =
          center.pixels[Math.floor(Math.random() * center.pixels.length)];
        const isAlien = Math.random() < 0.1;
        const edgePos = getRandomEdgePosition();
        particles.push(
          new Particle(edgePos.x, edgePos.y, pixel.x, pixel.y, isAlien, isS)
        );
      }
    });
