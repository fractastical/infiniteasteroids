...
const desktopTutorialSteps = [
  {
    text: "Use arrow keys or A,W,S,D keys to move your ship",
...
function getTutorialSteps() {
  ...
  else return fullscreenTutorialSteps;
}
...
function createTutorialAsteroidAndAddSecondary() {
  ...
  const activeWeapon = Object.values(secondaryWeapons).find(
...
function createTutorialElementalAsteroid() {
  ...
  if (!elementalAsteroidCreated && level > 1) {
    ...
    const randomType =
      elementalTypes[Math.floor(Math.random() * elementalTypes.length)];
...
function updateTutorial() {
  ...
  if (elementalAsteroidCreated && !elementalAsteroidDestroyed) {
    elementalAsteroidDestroyed = !asteroids.some(
      (asteroid) => asteroid.isElemental
    );
    if (elementalAsteroidDestroyed) createTutorialAlien();
  }

  // Check if tutorial asteroid is destroyed
  if (
    tutorialAsteroid &&
    !asteroids.includes(tutorialAsteroid) &&
    !tutorialAsteroidDestroyed
  ) {
