...
function shootTripleLaser() {
    ...
    // console.log("tripple");
    // Center laser
    const centerLaserX = ship.x + 50 * Math.sin(baseRotation * Math.PI / 180);
    const centerLaserY = ship.y - 50 * Math.cos(baseRotation * Math.PI / 180);
    ship.lasers.push({ x: centerLaserX, y: centerLaserY, rotation: baseRotation, size: (ship.laserLevel / 2) + 2 });

    // Left laser
    const leftLaserX = ship.x + 50 * Math.sin((baseRotation - 10) * Math.PI / 180);
    const leftLaserY = ship.y - 50 * Math.cos((baseRotation - 10) * Math.PI / 180);
    ship.lasers.push({ x: leftLaserX, y: leftLaserY, rotatiotn: baseRotation, size: (ship.laserLevel / 2) + 2 });

    // Right laser
    const rightLaserX = ship.x + 50 * Math.sin((baseRotation + 10) * Math.PI / 180);
    const rightLaserY = ship.y - 50 * Math.cos((baseRotation + 10) * Math.PI / 180);
