...
function createAsteroids(side) {
  if (currentMode === GameModes.ENDLESS_SLOW) {
    ...
    for (let i = 0; i < numberOfAsteroids; i++) {
      ...
      let isVerySmallAsteroid =
        Math.random() * 100 < chanceForVerySmallAsteroid;
      let isHardenedAsteroid = Math.random() * 100 < chanceForHardenedAsteroid;
      let isVeryHardenedAsteroid =
        Math.random() * 100 < chanceForVeryHardenedAsteroid;
      let isMegaHardenedAsteroid =
        Math.random() * 100 < chanceForMegaHardenedAsteroid;
      ...
      if (Math.min(innerWidth, innerHeight) > 700) {
        asteroidSize = isLargeAsteroid ? 80 : isSmallAsteroid ? 20 : 40;
      }
      ...
      if (!meteorMode) {
        ...
        if (spawnArea < 0.25) {
          ...
          x =
            canvas.width * 0.97 +
            Math.random() * (canvas.width * spawnPercentage);
          ...
          y =
            canvas.height * 0.97 +
            Math.random() * (canvas.height * spawnPercentage);
        } else {
          // Left edge
...
function drawAsteroids() {
  for (let i = 0; i < asteroids.length; i++) {
    ...
    if (asteroid.hitpoints > 100) {
      ...
      if (ringThickness > 0) {
        ...
        ctx.arc(
          asteroid.x,
          asteroid.y,
          Math.abs(asteroid.size - ringThickness - 2),
          0,
          Math.PI * 2
        );
    ...
    if (asteroid.frozen == true) ctx.strokeStyle = "#22EEEE";
...
function createEndlessSlowAsteroids() {
  ...
  let numberOfAsteroids =
    baseAsteroidCount + (wave - 1) * additionalAsteroidsPerWave;
  let spawnSide = 0;
  ...
  for (let i = 0; i < numberOfAsteroids; i++) {
    let hitpoints = Math.ceil(
      baseHitpoints + (wave - 1) * hitpointIncreasePerWave
    );

    let baseSpeed = 0.5;
    let speed =
      baseSpeed + (Math.random() * speedVariation * 2 - speedVariation);
    let baseAngle = 0;
    let angle =
      baseAngle + (Math.random() * angleVariation * 2 - angleVariation);
...
function createAsteroidCluster() {
  ...
  if (currentMode == GameModes.EASY) clusterSpeed *= 0.5;
  if (currentMode == GameModes.NORMAL) clusterSpeed *= 0.7;
  ...
  for (let i = 0; i < clusterSize; i++) {
    ...
    const dx = Math.cos(
      Math.atan2(
        targetCorner.y - startCorner.y,
        targetCorner.x - startCorner.x
      ) + angleOffset
    );
    const dy = Math.sin(
      Math.atan2(
...
function processAsteroidDeath(asteroid, fromArea = false) {
  ...
  if (wave > 55) baseDropChance = 0.0015; // 10% base chance to drop a gem
  if (wave > 55) baseDropChance = 0.0025; // 10% base chance to drop a gem
  if (wave > 45) baseDropChance = 0.005; // 10% base chance to drop a gem
  else if (wave > 35) baseDropChance = 0.01; // 10% base chance to drop a gem
  else if (wave > 25) baseDropChance = 0.02; // 10% base chance to drop a gem
  else if (wave > 15) baseDropChance = 0.04; // 10% base chance to drop a gem
  else if (wave > 7) baseDropChance = 0.07; // 10% base chance to drop a gem

  const hitpointFactor = 0.0005; // Increase drop chance by 0.05% per hitpoint
  const dropChance = Math.min(
    baseDropChance + asteroid.initialHitpoints * hitpointFactor,
    1
  );
  ...
  if (Math.random() < dropChance && droppedGems.length < 40) {
    ...
    if (gemType == "epic") {
      ...
      if (lastEpicWave > wave - 7) gemType = "rare";
      else lastEpicWave = wave;
    }
...
function addRareAsteroidToDisplay(type, color) {
  ...
  if (lastRareAsteroids.length === 3) {
    if (lastRareAsteroids.every((asteroid) => asteroid.type === "exploding")) {
      ...
      lastRareAsteroids.every((asteroid) => asteroid.type === "chainLightning")
    ) {
      triggerLightningStorm();
      lastRareAsteroids = [];
    } else if (
      lastRareAsteroids.every((asteroid) => asteroid.type === "acid")
    ) {
      triggerAcidRain();
      lastRareAsteroids = [];
    } else if (
      lastRareAsteroids.every((asteroid) => asteroid.type === "freezing")
    ) {
      triggerMegaFreeze();
      lastRareAsteroids = [];
    }
...
function drawMegaExplosions() {
  for (let i = megaExplosions.length - 1; i >= 0; i--) {
    ...
    if (explosion.duration % 5 === 0) {
      explosion.currentColorIndex =
        (explosion.currentColorIndex + 1) % explosion.colors.length;
    }
...
function drawRareAsteroidIndicators() {
  ...
  const padding = 10;
  const startX =
    canvas.width - (indicatorSize + padding) * maxRareAsteroidsDisplayed;
